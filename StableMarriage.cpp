...
bool ok(int q[], int c) { // Finish this code
	...
	static int wp[3][3] = { { 2,1,0 },		// Woman#0's preferences
	  ...
	  Second condition checks if (Man#i) and (Man#i's wife) both like each other more than their own spouse
	...
	*/
        for(int i = 0; i<c;i++){ //Checking if the women has already married or nah
           if(q[c]==q[i])
             return false;     
        }
        
        for(int i=0; i<c; i++){ //Checking if out of all possible combinations the two who are married cause no issues with another combination
...
//Decrement all numbers of 8 to 3
int main() {
   int c = 0;
   int q[3] = {0}; 
